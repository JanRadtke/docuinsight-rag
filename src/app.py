...
with st.sidebar:
    ...
    dev_mode = st.toggle("🛠️ Deep Logs", value=True, help="Shows all internal agent logs (planner, retrieval, quality gate, reader, writer). When enabled, all log sections are expanded by default. ⚠️ Do not toggle while a query is running — it will interrupt the agent.")
...
if "batch_discovery_report" in st.session_state:
    with st.expander("🔍 Batch Discovery Report", expanded=False):
        ...
        with col1:
            if st.button("🗑️ Delete Report", key="clear_batch_discovery"):
...
for idx, msg in enumerate(st.session_state.messages):
    with st.chat_message(msg["role"]):
        if msg["role"] == "assistant" and "extended_data" in msg:
            ...
            if dev_mode and gr:
                st.success(f"🛡️ Guardrail: **ALLOWED** — *{gr}*")
...
if prompt := st.chat_input("Ask a question about your documents..."):
    ...
    with st.chat_message("assistant"):
        ...
        if dev_mode and not is_blocked and block_reason:
            st.success(f"🛡️ Guardrail: **ALLOWED** — *{block_reason}*")
            st.session_state._guardrail_reason = block_reason

        if intent == "OFF_TOPIC":
            with st.expander("🛡️ Guardrail Decision", expanded=dev_mode):
            ...
            with st.status("🧠 Agent summarizing...", expanded=True) as status:
                ...
                if filename:
                    ...
                    if page_count > 80:
                        status.write("⚠️ Large document (>80 pages). Using Smart Sampling...")
                ...
                if est_tokens > 100000:
                    status.write(f"⚠️ Warning: Context very large ({int(est_tokens)} tokens). Truncating automatically...")
                    context = context[:350000]

                if not context.strip():
                    response = "❌ No documents found or the filter was too strict."
                else:
                    status.write("✍️ Drafting structured summary...")
                    try:
            ...
            with st.status("🔬 Comparison Agent analysing...", expanded=True) as status:
                ...
                if not filenames or len(filenames) < 2:
                    ...
                    if criteria:
                        ...
                        status.write("⚠️ No specific criteria found. Using default.")
                    ...
                    if not context.strip():
                        ...
                        try:
                            ...
                            with st.expander("🔮 Glass Box: Analysis Details", expanded=False):
                                st.markdown("**Internal Process Steps:**")
                                st.text(f"1. Scout: Identified {len(criteria) if criteria else 0} criteria.")
            ...
            if criteria and refs:
                with st.expander("📋 Comparison criteria used", expanded=False):
                    st.write("The Agent dynamically focused on the following areas:")
                    for c in criteria:
                        st.write(f"- {c}")
            ...
            with status_container.status("🧠 LangGraph Agent working (Parallel Mode)...", expanded=True) as status:
