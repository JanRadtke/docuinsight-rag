...
def refresh_sidebar_metrics() -> None:
...
def update_costs(input_text: str, output_text: str, input_tokens: int | None = None, output_tokens: int | None = None) -> None:
...
def retrieve_knowledge(question: str) -> tuple[str, list[dict], dict]:
    """Wrapper for Retriever.retrieve_knowledge()"""
    return st.session_state.retriever.retrieve_knowledge(question)


def retrieve_multiple_documents(filenames: list[str], total_token_budget: int = 100000, strategy: str = "balanced") -> tuple[str, list[dict], dict]:
...
def retrieve_intro_only(filenames: list[str], intro_pages: int = 3) -> tuple[str, list[dict], dict]:
    """Wrapper for Retriever.retrieve_intro_only()"""
    return st.session_state.retriever.retrieve_intro_only(filenames, intro_pages)


def get_document_page_count(filename: str) -> int:
    """Wrapper for Retriever.get_document_page_count()"""
    return st.session_state.retriever.get_document_page_count(filename)


def retrieve_full_document(filename: str | None = None, include_images: bool = False, structured: bool = False, intro_pages: int = 3, discussion_pages: int = 2, middle_pages: int = 0) -> tuple[str, list[dict], dict]:
